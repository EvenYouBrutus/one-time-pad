use otp_cipher::{decrypt, encrypt, generate_key};
...
fn main() {
    ...
    let key = generate_key(plaintext_bytes.len());

    // Encrypt the plaintext using OTP.
    let ciphertext = encrypt(plaintext_bytes, &key).expect("encryption failed");

    // Decrypt the ciphertext back into original plaintext.
    let decrypted_bytes = decrypt(&ciphertext, &key).expect("decryption failed");
